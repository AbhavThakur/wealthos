...
import { Sparkles, RefreshCw, ExternalLink, TrendingUp, TrendingDown, Minus } from "lucide-react";

// ── Demo data — shown in local dev when /api/market-data isn't available ──────
const DEMO_DATA = [
  { symbol: "^NSEI",  name: "Nifty 50",  price: 22400, change: 180,  changePct: 0.81,  currency: "INR" },
  { symbol: "^BSESN", name: "Sensex",    price: 73840, change: 540,  changePct: 0.74,  currency: "INR" },
  { symbol: "GC=F",   name: "Gold",      price: 2320,  change: -8,   changePct: -0.34, currency: "USD" },
  { symbol: "INR=X",  name: "USD / INR", price: 84.2,  change: -0.1, changePct: -0.12, currency: "INR" },
];
...
function useMarketSnapshot() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/market-data", { signal: AbortSignal.timeout(10000) });
  ...
  return {
    ...
    loading, lastSync, isDemo, refresh: load,
  };
}
...
const MOOD_CFG = {
  great: { emoji: "🚀", color: "#4ade80", bg: "#0d2b1a", border: "#22543d",
  ...
  good:  { emoji: "😊", color: "#86efac", bg: "#0f2318", border: "#166534",
  ...
  flat:  { emoji: "😐", color: "var(--text-secondary)", bg: "var(--surface)", border: "var(--border)",
  ...
  dip:   { emoji: "😌", color: "#fca5a5", bg: "#2b0d0d", border: "#7f1d1d",
  ...
  down:  { emoji: "😮", color: "#f87171", bg: "#3a0d0d", border: "#991b1b",
...
function StatTile({ quote, label, subtitle }) {
  ...
  return (
    <div className="card" style={{ padding: "1rem 1.1rem", display: "flex", flexDirection: "column", gap: 2 }}>
      <div style={{ fontSize: 12, color: "var(--text-secondary)", fontWeight: 600 }}>{label}</div>
      <div style={{ fontSize: 11, color: "var(--text-secondary)", marginBottom: 2 }}>{subtitle}</div>
      <div style={{ fontSize: 22, fontWeight: 700, fontFamily: "var(--font-display)", color: "var(--text)", lineHeight: 1.2 }}>
        {quote ? fmtNum(quote.price, quote.currency)
          : <span className="skeleton" style={{ width: 80, height: 22, display: "inline-block", borderRadius: 4 }} />}
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 2 }}>
        {quote ? (
          <><Icon size={13} style={{ color }} />
            <span style={{ fontSize: 13, color, fontWeight: 600 }}>{fmtPct(pct) || "—"}</span>
...
// ── Quick Links ───────────────────────────────────────────────────────────────
const QUICK_LINKS = [
  { label: "Google Finance",  url: "https://www.google.com/finance/",         emoji: "📊" },
  { label: "NSE India",       url: "https://www.nseindia.com/",                emoji: "📈" },
  { label: "Moneycontrol",    url: "https://www.moneycontrol.com/markets/",    emoji: "💹" },
  { label: "Screener.in",     url: "https://www.screener.in/",                 emoji: "🔍" },
  { label: "Value Research",  url: "https://www.valueresearchonline.com/",     emoji: "🏆" },
  { label: "BSE India",       url: "https://www.bseindia.com/",                emoji: "🏛️" },
];

// ── Main Page ─────────────────────────────────────────────────────────────────
export default function MarketPulse() {
  const { nifty, sensex, gold, usdinr, loading, lastSync, isDemo, refresh } =
    useMarketSnapshot();
  const { news, aiInsight, loading: newsLoading, aiLoading, generateInsight } =
    useMarketPulse();
  ...
  return (
    <div style={{ maxWidth: 860 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between",
        ...
        <button
          ...
          style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13,
            padding: "6px 12px", flexShrink: 0, marginTop: 4 }}
          onClick={refresh}
          disabled={loading}
        >
      ...
      {/* ── Market Mood Card ── */}
      {moodCfg && (
        <div style={{ background: moodCfg.bg, border: `1px solid ${moodCfg.border}`,
          borderRadius: "var(--radius-lg, 12px)", padding: "1.25rem 1.5rem",
          marginBottom: "1.25rem", display: "flex", alignItems: "flex-start", gap: 16 }}>
          <div style={{ fontSize: 40, lineHeight: 1, flexShrink: 0, marginTop: 2 }}>
            {moodCfg.emoji}
          </div>
          <div>
            <div style={{ fontSize: 18, fontWeight: 700, color: moodCfg.color,
              fontFamily: "var(--font-display)", marginBottom: 6 }}>
              {moodCfg.headline}
            </div>
            <p style={{ fontSize: 14, color: "var(--text)", lineHeight: 1.65, margin: 0 }}>
              {moodCfg.body}{goldNote}
            </p>
            <div style={{ marginTop: 10, fontSize: 12, color: "var(--text-secondary)", fontStyle: "italic" }}>
              💡 Tip: Short-term market moves don&apos;t derail long-term wealth. Stay consistent with your SIPs.
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)",
        ...
        <StatTile quote={nifty}  label="Nifty 50"  subtitle="Indian large-cap index" />
        <StatTile quote={sensex} label="Sensex"    subtitle="BSE top 30 companies" />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr",
        ...
        <div className="card" style={{ padding: "1.1rem" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
            ...
            >
              <RefreshCw size={10} style={aiLoading ? { animation: "spin 1s linear infinite" } : {}} />
          ...
          {aiLoading && (
            <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
              {[100, 85, 70].map((w) => (
                <div key={w} className="skeleton" style={{ height: 13, borderRadius: 4, width: `${w}%` }} />
              ))}
            </div>
          )}
          {!aiLoading && aiInsight && (
            <p style={{ fontSize: 13, color: "var(--text)", lineHeight: 1.65, margin: 0 }}>{aiInsight}</p>
          )}
          {!aiLoading && !aiInsight && (
        ...
        <div className="card" style={{ padding: "1.1rem" }}>
          ...
          {newsLoading && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {[1, 2, 3].map((i) => (
                <div key={i}>
                  <div className="skeleton" style={{ height: 13, borderRadius: 4, marginBottom: 4 }} />
          ...
          {!newsLoading && !news && (
            <p style={{ fontSize: 13, color: "var(--text-secondary)", margin: 0 }}>
              Add a <code style={{ fontSize: 12 }}>VITE_ALPHAVANTAGE_KEY</code> to see
              live finance news with sentiment.
            </p>
          )}
          {!newsLoading && news && (
            <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
              {news.slice(0, 4).map((a, i) => (
                <a key={i} href={a.url} target="_blank" rel="noopener noreferrer"
                  style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between",
                    gap: 8, padding: "8px 10px", borderRadius: "var(--radius-sm, 6px)",
                    background: "var(--surface)", border: "1px solid var(--border)",
                    textDecoration: "none", transition: "border-color 0.15s" }}
                  onMouseEnter={(e) => (e.currentTarget.style.borderColor = "var(--gold-border)")}
                  onMouseLeave={(e) => (e.currentTarget.style.borderColor = "var(--border)")}>
                  <div>
                    <div style={{ fontSize: 12, color: "var(--text)", lineHeight: 1.45, fontWeight: 500 }}>
                      {a.title}
                    </div>
